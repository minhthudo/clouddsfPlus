/*
 * ========================================================================
 * Cloud Decision Support Framework - CloudDSF dsamc.js - v1.0 - 2014/03/22
...
function drawPartitionLayout() {
  ...
  */  
  ...
  function partDPLabels() {
    return partitionPath.filter(function(d) {
      return "decisionPoint" == d.type;
            ...
            "class", "initDPText").append("svg:textPath").attr("xlink:href",
            function(d) {
              return "#path-" + d.id;
  ...
  function partLabels() {
    var i = -3.75;
    return layoutSVG.selectAll(".partitionLabel").data(
            layoutNodes.filter(function(d) {
              return "decision" == d.type;
            })).enter().append("svg:g").attr("transform", function(d) {
      return "rotate(" + ((i++ / 17) * 360) + ")translate(" + y(d.y) + ")";
    }).append("foreignObject").attr(
            "transform",
            function(d) {
              return d.id.toString()[0] == 4 || d.id.toString()[0] == 3
                      ? "rotate(180)" : null;
            }).attr("width", 90).attr("height", 60).attr("dx", "30px").attr(
            "x",
            function(d) {
              return d.id.toString()[0] == 4 || d.id.toString()[0] == 3
                      ? "-100px" : "10px";
            }).attr("dy", "-30px").attr("y", "-30px").append("xhtml:div").attr(
            "width", "100%").attr("height", "100%").append("xhtml:p").attr(
            "class",
            function(d) {
              return d.id.toString()[0] == 4 || d.id.toString()[0] == 3
                      ? "pathTextDecTurn" : "pathTextDec";
            }).html(function(d, i) {
      return d.label;
    }).on("click", function(d) {
      clickArc(d);
      ...
      layoutSVG
              ...
              .each(
                      ...
                      function(e, i) {
                        ...
                        if ("decRoot" == d.type && partitionLabels == null)
                          partitionLabels = partLabels();
                        if ("decRoot" == d.type && partitionDPLabels == null)
  ...
  var information = $('<div>')
          ...
          .append(
                  $('<div>')
                          ...
                          .append(
                                  $('<div>')
                                          .attr("class", "visInfoHeaderToggle")
                                          .append(
                                                  $('<a>')
                                                          .attr("href", "#")
                                                          .attr("class",
...
function mouseNodeClickPartition(n) {
  ...
  if ("decisionPoint" == n.type) {
    ...
    .append($('<div>').attr("class", "visInfoContentBOX").append(decOutcomeUL));
  ...
  function update(source) {
    ...
    nodeEnter.append("foreignObject").attr("width", 185).attr("height",
      ...
      return d.children || d._children ? "end" : "start";
            ...
            "class", function(d) {
              return "outcome" != d.type ? "treeTextDIV" : "treeOutTextDIV";
    ...
    link.enter().insert("svg:path", "g").attr("class", "treeLink").attr("d",
            function(d) {
              var o = {
                x: source.x0,
                y: source.y0
...
function drawClusterLayout() {
  ...
  */
  ...
  layoutCluster = d3.layout.cluster().size([360, clusterRY * 0.85 - 150])
          .children(function(d) {
            if (d.type == "decisionPoint") return d.children;
            if (d.type == "taskRoot") return d.children;
  ...
  layoutSVG = d3.select("#" + visContentID).style(
          "-webkit-backface-visibility", "hidden").append("svg").attr("width",
          clusterSVGWidth).attr("height", clusterSVGHeight).append("g").attr(
          "transform", "translate(" + clusterRX + "," + clusterRY + ")");
  ...
  /* === Texts === */
  var textNodes = layoutSVG.selectAll("g.clusterNode").data(pathNodes).enter()
          .append("svg:g").attr("id", function(d, i) {
            ...
            return d.label;
          }).on("mouseover", clusterMouseover).on("mouseout", clusterMouseout)
  ...
  var clusterGroups = layoutSVG.selectAll("g.clusterGroup")
          .data(decisionPoints).enter().append("svg:g").attr("class",
                  "clusterGroup");

  clusterGroups.append("svg:path").attr("id", function(d) {
    return "n-" + d.id;
  }).attr(
          "class",
          function(d) {
            return "decisionPoint" == d.type ? "clusterGroupPath decisionPoint"
                    + d.id.toString()[0] : "clusterGroupPath task";
          }).attr("d", function(d) {
  ...
  function setClusterLegend() {
    ...
    legendItems.append($('<span>').attr("class", "legendItem small").append(
            $('<img>').attr("src", "./img/legends/cluster-line.png")).append(
            $('<span>').text("Influence / Affects Link")));
    legendItems.append($('<span>').attr("class", "legendItem small").append(
            $('<img>').attr("src",
                    "./img/legends/cluster-lineAffectHighlight.png")).append(
            $('<span>').text("Highlighted Affects Link")));
    legendItems.append($('<span>').attr("class", "legendItem small").append(
            $('<img>').attr("src",
...
function mouseNodeClickCluster(n) {
                                                    ...
                                                    $('<span>')
                                                            ...
                                                            .text(
                                                                    function() {
                                                                      ...
                                                                      } else if ("both" == d.dir) { return "is bidirectional affected"; }
                                                                    }))));
  ...
  .append($('<div>').attr("class", "visInfoContentBOX").append(relatedTasksUL));
  ...
  if ("decision" == n.type) {
    ...
    var influencedDecisionsUL = $('<ul>')
            .attr("class", "visInfoRelatedNodesUL");

    var show = false;
    influencedDecisions.each(function(d) {

      influencedDecisionsUL.append($('<li>').append(
              $('<span>').text(d.target.label).attr("class", function() {
    ...
    influencingDecisions.each(function(d) {
      ...
      influencingDecisionsUL.append($('<li>').append(
              $('<span>').text(d.source.label).attr("class", function() {
  ...
  layoutSVG.select("#n-" + n.id).select("text").classed(
          ...
          function(d) {
            return layoutSVG.select("#n-" + n.id).classed("noteHighlighted")
                    ? true : false;
          });
}
...
function drawForceLayout() {
  ...
  */
  ...
  layoutSVG.append("defs").selectAll("marker").data(
          ["influenceStart", "influenceEnd", "affectStartDec", "affectEndDec",
              "affectStartTask", "affectEndTask"]).enter().append("marker")
          .attr("id", function(d) {
            return d;
          ...
          .attr("orient", "auto").append("path").attr("class", function(d) {
            ...
            } else if (d.indexOf("affect") != -1) { return "affectMarker"; }
          }).attr("d", function(d) {
            if (d.indexOf("Start") != -1) {
              return "M10,5 L0,0 L10,-5";
            } else if (d.indexOf("End") != -1) { return "M0,-5 L10,0 L0,5"; }
          });
  ...
  decisionPoints.append("svg:g").attr(
          "transform",
          function(d) {
            return (d.id % 2) == 0 ? "rotate(90)translate(0,-" + dpRectHeight
                    * 0.8 + ")" : "translate(0," + dpRectHeight * 0.2 + ")";
  ...
  decisions.append("svg:g").append("foreignObject").attr("width", function(d) {
    return decRecWidth;
  }).attr("height", function(d) {
    return decRecHeight;
  }).append("xhtml:div").attr("width", "100%").attr("height", "100%").attr(
  ...
  function onTick() {
    ...
    pathLinksTaskRels.attr(
            "x1",
            function(d) {
              return ("decision" == d.source.type)
                      ? (d.source.x + decRecWidth / 2) : d.source.x;
            }).attr(
            "y1",
            function(d) {
              return ("decision" == d.source.type)
                      ? (d.source.y + decRecHeight / 2) : d.source.y;
            }).attr(
            "x2",
            function(d) {
              return ("decision" == d.target.type)
                      ? (d.target.x + decRecWidth / 2) : d.target.x;
            }).attr(
            "y2",
            function(d) {
              return ("decision" == d.target.type)
                      ? (d.target.y + decRecHeight / 2) : d.target.y;
            });

    // Hierarchy Links
    lineLinkHierarchy.attr(
            "x1",
            function(d) {
              if ("decisionPoint" == d.source.type) {
                if ((d.source.id % 2) != 0) {
                  return d.source.x + dpRectWidth / 2;
                } else if ((d.source.id % 2) == 0) { return d.source.x
                        + dpRectHeight / 2; }
            ...
            function(d) {
              if ("decisionPoint" == d.source.type) {
                ...
                } else if ((d.source.id % 2) == 0) { return d.source.y
                        + dpRectWidth / 2; }
              } else {
                return d.source.y;
              }
  ...
  function setForceLegend() {
    ...
    legendItems.append($('<span>').attr("class", "legendItem small").append(
            $('<img>').attr("src", "./img/legends/force-affectationLink.png"))
...
function mouseNodeClickForce(n) {
                                                              ...
                                                              $('<span>')
                                                                      ...
                                                                      .text(
                                                                              function() {
                                                                                ...
                                                                                } else if ("both" == v.dir) { return "is bidirectional affected"; }
                                                                              }))));
                    });
  ...
  selLinks.each(function(d) {
    ...
    if ("taskRel" == d.type) {
      if (layoutSVG.select("#n-" + n.id).classed("noteHighlighted")) {
        if ("decision" == d.source.type)
          layoutSVG.select("#d-" + d.source.id).classed("forceHighlightNode",
                  true);
        if ("decision" == d.target.type)
          layoutSVG.select("#d-" + d.target.id).classed("forceHighlightNode",
                  true);
        if ("task" == d.source.type)
          layoutSVG.select("#t-" + d.source.id).classed("forceHighlightNode",
                  true);
        if ("task" == d.target.type)
          layoutSVG.select("#t-" + d.target.id).classed("forceHighlightNode",
                  true);
      } else {
        if ("decision" == d.source.type)
          layoutSVG.select("#d-" + d.source.id).classed("forceHighlightNode",
...
function mouseOverForce(d) {
  layoutSVG
          .selectAll(".forceLinkSource-" + d.id + ", .forceLinkTarget-" + d.id)
          .classed("forceAffectationLinkHighlightHover", function(d) {
            return "taskRel" == d.type ? true : null;
          });
...
function drawTreemapLayout() {
  ...
  grandparent.append("rect").attr("y", -margin.top).attr("width", width).attr(
          "height", margin.top);
  ...
  function accumulate(d) {
    return (d._children = d.children) ? d.value = d.children.reduce(function(p,
            v) {
...
function setInfoLabel(d) {
  ...
  visInfoLabelBOX.find('#visInfoLabelTextBOX h4').text(d.label).addClass(
          function() {
            ...
            } else if ("outcome" == d.type) { return "textcolorOutcome"
                    + d.id.toString()[0]; }
          });
