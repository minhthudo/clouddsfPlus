...
var kbNavigator = (function() {
    ...
    var config = {
        ...
        // Text for Outcome relations)
        legendRelations: ["Including", "Excluding", "Requiring", "Allowing",
            "Conditional Allowing", "Allowing Plus", "Arbitrary Allowing"
        ],
        relations: ["in", "ex", "requiring", "a", "ca", "ap", "aa"],
        legendNoItemsPerRow: 2,
        legendWidthFactor: 15,
        legendStepX: 2,
        legendOffsetX1: 0.5,
        legendOffsetX2: 2,
        legendStartY: 1,
    ...
    var root, initialNodes, requiringLinks, requiringLines = [],
        outcomeLinks, outcomePaths = [];
    ...
    function initialize() {
        ...
        legend.selectAll("line").data(config.relations).enter()
            .append("line")
            ...
            .attr("x1", function(d, i) {
                var col = i % config.legendNoItemsPerRow;
                return (mC.iWidth / config.legendWidthFactor) * ((col * config.legendStepX) + config.legendOffsetX1);
            })
            .attr("y1", function(d, i) {
                var row = Math.floor(i / config.legendNoItemsPerRow);
                var pos = config.legendStartY + row * config.legendStepY;
                return pos + "em";
            })
            .attr("x2", function(d, i) {
                var col = i % config.legendNoItemsPerRow;
                return (mC.iWidth / config.legendWidthFactor) * ((col * config.legendStepX) + config.legendOffsetX2);
            })
        ...
        legend.selectAll("text").data(config.legendRelations).enter()
            ...
            .attr("x", function(d, i) {
                var col = i % config.legendNoItemsPerRow;
                return (mC.iWidth / config.legendWidthFactor) * ((col * config.legendStepX) + 0.5 * (config.legendOffsetX1 + config.legendOffsetX2));
            }).attr("y", function(d, i) {
                var row = Math.floor(i / config.legendNoItemsPerRow);
                var pos = config.legendStartY + row * config.legendStepY;
                return pos + "em";
        ...
        .size([mC.panelWidth, mC.panelHeight])
        .charge(
            function(d) {
                return setCharge(d);
            }
    ...
    function update() {
        ...
        // initialize tooltip for paths (links)
        pathTooltip = d3.tip()
        .attr('class', 'd3-tip')
        .direction('nw')
        .offset(
            function() {
                ...
                // offset from the relative mouse position
                var offset = [30, 30];
                var x = coordinates[0] - bbox.x - offset[0];
                var y = coordinates[1] - bbox.y - offset[1];
                return [y, x];
            }
        )
        .html(
            function(d) {
                return generatePathTooltip(d);
            }
        );

        // invoke the tip in the context of your visualization
        pathGroup.call(pathTooltip);
        ...
        .on('mouseover', pathTooltip.show)
        .on('mouseout', pathTooltip.hide);
        ...
        .html(
            function(d) {
                return generateNodeTooltip(d);
            }
        );

        // Invoke tip in context of visualization
        nodeGroup.call(nodeTooltip);
        ...
        .on("mouseover", nodeTooltip.showTransition)
        .on("mouseout", nodeTooltip.hideDelayed);
    ...
    function setLabelYshift(d) {
        ...
        if ((d.target.x >= (d.source.x + 20)) || (d.target.x <= (d.source.x - 20))) {
            return (0.5 * 0.85 / 2) +
                "em";
        }
        // 30 if below
            ...
            function(link) {
                ...
                if (source.highlighted === true) {
                    ...
                    var newGraphLink = new graphLink(source, target,
                        link.relationGroup, link.type, link.explanation, link.active, link.conflict);
        ...
        if (config.requiring === true) {
            requiringLinks.forEach(function(link) {
                if (link.active === true) {
                    ...
                    var newGraphLink = new graphLink(source, target,
                        link.relationGroup, link.type, link.explanation, false, false);
    ...
    function resetDecisionOutcomes(tempNode) {
        ...
        // EXCEPTION: outcomes of "SCV" decision are always selectable,
        // so they shouldn't be reset here
        var parent = tempDecNodes_lookup[tempNode.parent];
        if (parent.abbrev != "SCV") {
            tempNodes.filter(function(d) {
                if (tempNode.outGroup == d.outGroup && d.id != tempNode.id) return d;
    ...
    function initializeNode() {
        ...
        initialNodes.forEach(function(d) {
            ...
            if (d.type == "dec") {
                var newTempDecNode = new tempDecNode(d.id, d.type, d.label, d.abbrev, d.parent);
    ...
    /**
     * Generate tooltip text for outcome relations (paths).
     *
     * @memberOf outcomeGraph
     * @param d
     *          hovered path
     */
    function generatePathTooltip(d) {
        return '<p><strong>Source: </strong>' + d.source.abbrev + '</p>' +
            '<p><strong>Target: </strong>' + d.target.abbrev + '</p>' +
            '<p><strong>Relation: </strong>' + config.legendRelations[config.relations.indexOf(d.type)] + '</p>' +
     ...
     *          hovered node
     */
     function generateNodeTooltip(d) {
        ...
        switch (d.type) {
            case "dec":
                // decision
                var classification = '<p><strong>Classification: </strong>' + d.classification + '</p';
                return mainText + classification;
            case "out":
                // outcome
                var parent = node_lookup[d.parent];
                if (parent.abbrev == "SCV") {
                    // only outcomes of "SCV" decision have URL and Service Typpe
                    var url = '<p><strong>URL: </strong>' + d.url + '</p>';
                    var serviceType = '<p><strong>Service Type: </strong>' + d.serviceType + '</p>';
                    return mainText + url + serviceType;
                } else {
                    return mainText;
                }
            default:
     ...
     */
    function graphLink(source, target, relationGroup, type, explanation, active, conflict) {
        this.source = source;
        this.target = target;
    ...
    function tempNode(outGroup, id, type, label, parent) {
        ...
        /**
         * Reset outcome enable selection
         * ATTENTION: to allow more than one outcomes to be selected for the SCV decision
         */
        this.setOutcomeSelectable = function() {
            this.selectable = true;
            this.excluded = false;
            this.conflicting = false;
        };
     ...
     */
    function tempDecNode(id, type, label, abbrev, parent) {
        this.decided = false;
