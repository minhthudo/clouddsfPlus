...
var outcomeGraph = (function() {
    ...
    var config = {
        ...
        legendRelations: ["Including", "Excluding", "Allowing", "Conditional Allowing", "Allowing Plus", "Arbitrary Allowing"],
        relations: ["in", "ex", "a", "ca", "ap", "aa"],
        start: true,

        legendStep: 1.5,
        legendOffsetStart: 0.5,
        legendOffsetEnd: 1.75
    };
    ...
    function initialize() {
        ...
        legend.selectAll("line").data(config.relations).enter().append("line")
            ...
            .attr("x1", function(d, i) {
                return (mC.iWidth / 10) * ((i * config.legendStep) + config.legendOffsetStart);
            })
            .attr("y1", 0)
            .attr("x2", function(d, i) {
                return (mC.iWidth / 10) * ((i * config.legendStep) + config.legendOffsetEnd);
            })
            .attr("y2", 0)
        ...
        legend.selectAll("text").data(config.legendRelations).enter()
            .append("text")
            .attr("x", function(d, i) {
                return (mC.iWidth / 10) * ((i * config.legendStep) + 0.5 * (config.legendOffsetStart + config.legendOffsetEnd));
            })
            .attr("y", 0)
            .attr("dy", "2em")
        ...
        outcomeLinks.filter(function(d) {
            ...
            if (typeof source !== 'undefined') {
                ...
                if (source.highlighted) {
                    ...
                    newLink.explanation = link.explanation;
