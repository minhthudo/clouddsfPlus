...
var layoutPartitionDefaultSettings = {"showLabels" : true, "showTooltips" : false};

// Tree Layout
var layoutTree = null;
var layoutTreeDefaultSettings = {"showLabels" : true, "showTooltips" : false};
...
var layoutForceDefaultSettings = {"showDecisions" : true, "showTasks" : true, "showInfluenceLinks" : false, "showAffectLinks" : true, "showLabels" : true, "showTooltips" : false};

// Treemap Layout
var layoutTreemap;
var layoutTreemapDefaultSettings = {"showLabels" : true, "showTooltips" : false};
...
function toogleSettingsBox(endState, duration) {
    if("collapsed" == endState) {
        if(!$('#visPanelSettingsContent').is(":hidden")) $('#visPanelSettingsContent').slideToggle('fast').delay(duration);
...
function drawPartitionLayout(){
    ...
    /* === Init SVG === */
    layoutSVG = d3.select("#"+visContentID)
        .append("svg")
            .attr("width", partSVGWidth )
            .attr("height", partSVGHeight )
        .append("g")
            .attr("transform", "translate(" + partSVGWidth/2 + "," + partSVGHeight/2 + ")");

    /* === Init Partition === */
    layoutPartition = d3.layout.partition()
            .sort(null)
            //.size([2 * Math.PI, partRadius ]) 
            .value(function(d) { return d.weight ? d.weight : 1; });
    ...
    var partitionPath = layoutSVG.selectAll(".partitionPath")
        .data(layoutNodes).enter()
            .append("svg:g")
                .attr("id", function(d, i) {return "n-" + d.id;})
                .attr("class", function(d, i) {return "gPartPath";})
                .on("click", function(d){
                    clickArc(d);
    ...
    partitionPath.append("svg:path")
        ...
        function partDPLabels() {
            ...
            return partitionPath.filter(function(d){return "decisionPoint" == d.type; })
            .append("svg:g").append("svg:text")
                .attr("dx", 25)
                .attr("dy", 50)
                .attr("class", "initDPText")
                    .append("svg:textPath")
                        .attr("xlink:href", function(d) { return "#path-" + d.id; })
                        .attr("class", "pathTextDP")
                        .text(function(d) { return d.id == 3 ? "Multi-Tenancy" : d.label; });
        ...
        function partLabels() {
            var i = -3.75;
            return layoutSVG.selectAll(".partitionLabel")
            .data(layoutNodes.filter(function(d){
            return "decision" == d.type; })).enter()
               .append("svg:g")
               .attr("transform", function(d) { return "rotate(" + ((i++/17) * 360) + ")translate(" + y(d.y) + ")"; })
                    .append("foreignObject")
                    .attr("transform", function(d) { return d.id.toString()[0] == 4 || d.id.toString()[0] == 3 ? "rotate(180)" : null; })
                    .attr("width", 90)
                    .attr("height", 60)
                    .attr("dx", "30px" )
                    .attr("x", function(d) { return d.id.toString()[0] == 4 || d.id.toString()[0] == 3 ? "-100px" :"10px"; } )
                    .attr("dy", "-30px")
                    .attr("y", "-30px")
                        .append("xhtml:div")
                        .attr("width", "100%")
                        .attr("height", "100%") 
                            .append("xhtml:p")
                            .attr("class", function(d) { return d.id.toString()[0] == 4 || d.id.toString()[0] == 3 ? "pathTextDecTurn" :"pathTextDec"; })
        ...
        var text = partitionPath.append("text")
        .attr("class", "pathTextDec")
  ...
  function computeTextRotation(d) {
    ...
    function clickArc(d) {
        ...
        if("outcome" != d.type) {
            ...
            layoutSVG.selectAll("path").transition().duration(750).attrTween("d", arcTween(d)).each("end", function(e, i) {
              // check if the animated element's data e lies within the visible angle span given in d
              if (e.x >= d.x && e.x < (d.x + d.dx)) {
                // get a selection of the associated text element
                var arcText = d3.select(this.parentNode).select(".pathTextDec");
                if("decisionPoint" == e.type) console.log("DP");
                // fade in the text element and recalculate positions
                arcText.transition().duration(250)
                  .attr("opacity", function(od) { return ("outcome" == od.type && "decision" != d.type) || "decRoot" == d.type ? 0 : 1;})
                  .attr("transform", function() { return "rotate(" + computeTextRotation(e) + ")"; })
                  .attr("x", function(d) { return y(d.y); });
              }
              
              if("decRoot" == d.type && partitionLabels == null) partitionLabels = partLabels();
              if("decRoot" == d.type && partitionDPLabels == null) partitionDPLabels = partDPLabels();
    ...
    function arcTween(d) {
        ...
        var xd = d3.interpolate(x.domain(), [d.x, d.x + d.dx]),
            ...
            yr = d3.interpolate(y.range(), [d.y ? 20 : 0, partRadius]);
        return function(d, i) {
            return i
...
function setPartitionDefaultInformation() {
    ...
    var defaultInformation = $('<div>').attr("class", "visInfoBOX")
            ...
            .text("Click on an arcs to zoom in the specific element. Click on the center circle to zoom out."))
            
            .append($('<p>').attr("class", "defaultInformation")
            .text("This panel then shows detailed information about the selected element."));  
...
function setPartitionSettings() {
    ...
    var information = $('<div>').attr("class", "visInfoBOX")
        .append($('<div>').attr("class", "visInfoHeaderBOX")
            ...
            .append($('<div>').attr("class", "visInfoHeaderToogle")
                            .append($('<a>').attr("href", "#").attr("class", "visInfoHeaderToogleLink").append($('<span>').attr("class", "glyphicon glyphicon-minus"))))
                        .append($('<div>').attr("class", "clearfix")))
            
        .append($('<div>').attr("class", "visInfoContentBOX")
        
            .append($('<div>').attr("class", "input-group input-group-sm visInputBox")
                .append($('<span>').attr("class", "input-group-addon")
                    .append($('<input>').attr("type", "checkbox").attr("name", "showLabels").attr("id", "showLabels").attr("checked", function(){return layoutPartitionDefaultSettings["showLabels"];})))
                .append($('<span>').attr("class", "form-control").text("Show Labels"))
            )
        );
    ...
    $('#showLabels').change(function(){
        if($(this).prop("checked")){
            $('.pathTextDP, .pathTextDecTurn, .pathTextDec').css("display", "block");
        } else {
...
function mouseNodeClickPartition(n) {
    ...
    // Exiting node Catch
    if((!n.children && !"outome" == n.type) || "root" == n.type || "decRoot" == n.type)  {
        clearVisRightContent();
        hideInfoLabel();
        setPartitionDefaultInformation();
        return;
    ...
    if ("decisionPoint" == n.type) {
        ...
        $.each(n.children, function(i,v){
             ...
             dpNodeInfosUL.append($('<li>')
                .append($('<span>')
                .attr("class", function(){ return "bordercolorDecision" + v.id.toString()[0]; })
                .text(v.label)));
        
        });
                
        var nodeInfos = $('<div>').attr("class", "visInfoBOX")
            .append($('<div>').attr("class", "visInfoHeaderBOX")
                .append($('<h4>').text("Decisions"))
                .append($('<div>').attr("class", "visInfoHeaderToogle")
                            .append($('<a>').attr("href", "#").attr("class", "visInfoHeaderToogleLink").append($('<span>').attr("class", "glyphicon glyphicon-minus"))))
                        .append($('<div>').attr("class", "clearfix")))
                
            .append($('<div>').attr("class", "visInfoContentBOX")
                .append(dpNodeInfosUL)
            );  
        ...
        var nodeInfos = $('<div>').attr("class", "visInfoBOX")
            ...
            .append($('<div>').attr("class", "visInfoContentBOX")
                .append($('<ul>').attr("class", "visInfoNodeInformationUL")
                    .append($('<li>')
                        .append($('<span>').text("Decision Point:"))
                        .append($('<span>').text(n.parent.label))
                    )
                    .append($('<li>')
                        .append($('<span>').text("Classification:"))
                        .append($('<span>').text(n.parent.classification))
                    )
                )
        ); 
        $('#visRightContent').append(nodeInfos);
        ...
        $.each(n.children, function(i,v){
             ...
             decOutcomeUL.append($('<li>')
                .append($('<span>')
                .attr("class", function(){ return "bordercolorOutcome" + v.id.toString()[0]; })
                .text(v.label)));
        
        });
        
        var decOutcomeInfos = $('<div>').attr("class", "visInfoBOX")
            .append($('<div>').attr("class", "visInfoHeaderBOX")
                .append($('<h4>').text("Possible Outcomes"))
                .append($('<div>').attr("class", "visInfoHeaderToogle")
                            .append($('<a>').attr("href", "#").attr("class", "visInfoHeaderToogleLink").append($('<span>').attr("class", "glyphicon glyphicon-minus"))))
                        .append($('<div>').attr("class", "clearfix")))
...
function drawTreeLayout() {
    ...
    function update(source) {
        ...
        // Update the nodes…
        var node = layoutSVG.selectAll(".treeNode")
        ...
        nodeEnter.append("foreignObject")
              .attr("width", 185)
              .attr("height", function(d) { return d.label.length < 25 ? 20 : 40; })
              .attr("x", function(d) { return d.children || d._children ? -200 : 20; })
              .attr("y", function(d) { return d.label.length < 25 ? "-9px" : "-18px";})
              .attr("text-anchor", function(d) { return d.children || d._children ? "end" : "start"; })
                    .append("xhtml:div")
                        .attr("width", "100%")
                        .attr("height", "100%")
                        .attr("class", function(d) {return "outcome" != d.type ? "treeTextDIV" : "treeOutTextDIV" ;})
                            .append("xhtml:p")
                            .html(function(d, i) { return d.label; });
        ...
        // Update the links…
        var link = layoutSVG.selectAll("path.treeLink")
                .data(layoutTree.links(nodes), function(d) {
                    return d.target.id;
                });
...
function setTreeDefaultInformation() {
    ...
    var defaultInformation = $('<div>').attr("class", "visInfoBOX")
            .append($('<p>').attr("class", "defaultInformation")
            .text("Click on a node to either expand or collapse its sub-nodes."))
...
function setTreeSettings() {
    ...
    var information = $('<div>').attr("class", "visInfoBOX")
        ...
        .append($('<div>').attr("class", "visInfoContentBOX")
            ...
            .append($('<div>').attr("class", "input-group input-group-sm visInputBox")
                .append($('<span>').attr("class", "input-group-addon")
                    .append($('<input>').attr("type", "checkbox").attr("name", "showLabels").attr("id", "showLabels").attr("checked", function(){return layoutTreeDefaultSettings["showLabels"];})))
                .append($('<span>').attr("class", "form-control").text("Show Labels"))
            )
        );
...
function drawClusterLayout() {
    ...
    /* === Init Cluster / Bundle === */
    layoutCluster = d3.layout.cluster()
        .size([360, clusterRY*0.85 - 150]);
    ...
    layoutSVG = d3.select("#"+visContentID)
        .style("-webkit-backface-visibility", "hidden")
            .append("svg")
                .attr("width",clusterSVGWidth)
                .attr("height",clusterSVGHeight)
            .append("g")
                .attr("transform", "translate(" + clusterRX + "," + clusterRY + ")");

    /* === Nodes / Links ===*/
    var nodes = layoutCluster.nodes(clusterNodesTree);
    var pathNodes = nodes.filter(function(d){return ("decision" == d.type || "task" == d.type);});
    ...
    /* === Paths === */
    var path = layoutSVG.selectAll("path.clusterLink")
        .data(links).enter()
            .append("svg:path")
                .attr("class", function(d){ return "clusterLink source-"+ d.source.id + " target-"+ d.target.id; })
                .attr("d", function(d, i) { return line(splines[i]); });
    
    /* === Texts === */
    var textNodes = layoutSVG.selectAll("g.clusterNode")
        .data(pathNodes).enter()
        .append("svg:g")
            .attr("id", function(d, i) {return "n-" + d.id;})
            .attr("class", function(d){
                if("decision" == d.type) {
            ...
            .attr("transform", function(d) { return "rotate(" + (d.x - 90) + ")translate(" + d.y + ")"; })
                .append("svg:text")
                    .attr("dx", function(d) { return d.x < 180 ? 38 : -38; })
                    .attr("dy", ".31em")
            ...
            .on("click", mouseNodeClickCluster); 
    
    /* === Arc Blueprint  === */
    var arc = d3.svg.arc()
        .innerRadius(clusterRY*0.85 - 150)
        .outerRadius(clusterRY*0.85 - 120);
             
    /* === Decision Points - Groups === */
    var decisionPoints = nodes.filter(function(d){return ("decisionPoint" == d.type || "taskRoot" == d.type);});
    
    var clusterGroups = layoutSVG.selectAll("g.clusterGroup")
        .data(decisionPoints).enter()
        .append("svg:g")
            .attr("class", "clusterGroup");
            
    clusterGroups.append("svg:path")
            .attr("id", function(d){ return "n-" + d.id;})
            .attr("class", function(d){ return "decisionPoint" == d.type ? "clusterGroupPath decisionPoint" + d.id.toString()[0] : "clusterGroupPath task" ;})
            .attr("d", function(d){
     ...
     clusterGroups.append("svg:text")
            .attr("dx", 10)
            .attr("dy", 20)
                .append("svg:textPath")
                    .attr("class", function(d) {return "taskRoot" == d.type ? null : "clusterGroupDPText";})
                    .attr("xlink:href", function(d) { return "#n-" + d.id; })
                .text(function(d) { return d.id == 3 ? "MT" : d.label; });
            

    /* === Cluster Functions === */
    function clusterMouseover(d) {
    
        layoutSVG.selectAll("path.clusterLink.source-" + d.id + ", path.clusterLink.target-"  + d.id)
            .classed("clusterInfluenceLink", function(d){ return "DecRel" == d.type ? true : null;})
            .classed("clusterAffectationLink", function(d){ return "taskRel" == d.type ? true : null;});
    }

    function clusterMouseout(d) {

        layoutSVG.selectAll("path.clusterLink.source-" + d.id + ", path.clusterLink.target-"  + d.id)
            .classed("clusterInfluenceLink", false)
            .classed("clusterAffectationLink", false);        
    }
    ...
    function setClusterLegend() {
        ...
        legendItems.append($('<span>').attr("class", "legendItem")
            .append($('<img>').attr("src", "./img/legends/cluster-line.png"))
            .append($('<span>').text("Influence / Affects Link"))
            );
        legendItems.append($('<span>').attr("class", "legendItem")
            .append($('<img>').attr("src", "./img/legends/cluster-lineAffectHighlight.png"))
...
function setClusterDefaultInformation() {
    ...
    var defaultInformation = $('<div>').attr("class", "visInfoBOX")
            .append($('<p>').attr("class", "defaultInformation")
            .text("Hover over a text label to highlight the elements relationships. Click on one or more text labels to either select or deselect it and its relationships."))
...
function setClusterSettings() {
    ...
    var clearHighlight = $('<div>').attr("class", "visInfoBOX")
        .append($('<div>').attr("class", "visInfoHeaderBOX")
            .append($('<h4>').text("Clear Highlights"))
            .append($('<div>').attr("class", "visInfoHeaderToogle")
                            .append($('<a>').attr("href", "#").attr("class", "visInfoHeaderToogleLink").append($('<span>').attr("class", "glyphicon glyphicon-minus"))))
                        .append($('<div>').attr("class", "clearfix")))
        ...
        .append($('<div>').attr("class", "visInfoContentBOX")
            ...
            .append($('<div>').attr("class", "visInputBox")
                .append($('<button>').attr("type", "button").attr("class", "btn btn-default btn-sm btn-block").attr("id", "clearClusterHighlights").text("Clear Highlights"))   
            )
        );
...
function clearClusterHighligths() {
    ...
    layoutSVG.selectAll('.noteHighlighted, .clusterHighlightedTextNode, .clusterAffectationLinkClick, .clusterInfluenceLinkClick')
        .classed("noteHighlighted", false)
        .classed("clusterHighlightedTextNode", false)
        .classed("clusterAffectationLinkClick", false)
        .classed("clusterInfluenceLinkClick", false); 
...
function mouseNodeClickCluster(n) {
    ...
    // Set Tasks Relations
        ...
        var relatedTasks = layoutSVG.selectAll("path.clusterLink.source-" + n.id + ", path.clusterLink.target-"  + n.id)
            .filter(function(d){return "task" == d.source.type || "task" == d.target.type;});
        
        var relatedTasksUL = $('<ul>').attr("class", "visInfoRelatedNodesUL");
        
        relatedTasks.each(function(d){
            
            relatedTasksUL.append($('<li>')
                .append($('<span>')
                .attr("class", function(){
                    
                    if(d.source.id != n.id) {
                        if("task" == d.source.type) return "bordercolorTask";
                        if("decision" == d.source.type) return "bordercolorDecision" + d.source.id.toString()[0];
                    } else {
                        if("task" == d.target.type) return "bordercolorTask";
                        if("decision" == d.target.type) return "bordercolorDecision" + d.target.id.toString()[0];
                    } 
                })
                .text(function(){
                    return d.source.id != n.id ? d.source.label : d.target.label;
                })
                    .append($('<span>').attr("class", "affectDirection").text(function(){
                        if("auto" == d.dir) {
                            if(d.source.id != n.id) {
                                return "is affecting";
                            } else {
        ...
        var relatedLabel = "";
        if("decision" == n.type) relatedLabel = "Related Tasks";
        if("task" == n.type) relatedLabel = "Related Decisions";
        
        var relatedTasksInfoBOX = $('<div>').attr("class", "visInfoBOX")
            .append($('<div>').attr("class", "visInfoHeaderBOX")
                .append($('<h4>').text(relatedLabel))
                .append($('<div>').attr("class", "visInfoHeaderToogle")
                            .append($('<a>').attr("href", "#").attr("class", "visInfoHeaderToogleLink").append($('<span>').attr("class", "glyphicon glyphicon-minus"))))
                        .append($('<div>').attr("class", "clearfix")))
    ...
    if("decision" == n.type) {
        ...
        var influencedDecisions = layoutSVG.selectAll("path.clusterLink.source-" + n.id)
                .filter(function(d){ return "DecRel" == d.type;});
                

            var influencedDecisionsUL = $('<ul>').attr("class", "visInfoRelatedNodesUL");
        
            var show = false;
            influencedDecisions.each(function(d){
            
                influencedDecisionsUL.append($('<li>')
                    .append($('<span>').text(d.target.label).attr("class",function(){
                        return "bordercolorDecision" + d.target.id.toString()[0];
                    })));
                show = true;
            ...
            if(show) {
                var influencedDecisionsInfos = $('<div>').attr("class", "visInfoBOX")
                    .append($('<div>').attr("class", "visInfoHeaderBOX")
                        .append($('<h4>').text("Influenced Decisions"))
                        .append($('<div>').attr("class", "visInfoHeaderToogle")
                            .append($('<a>').attr("href", "#").attr("class", "visInfoHeaderToogleLink").append($('<span>').attr("class", "glyphicon glyphicon-minus"))))
                        .append($('<div>').attr("class", "clearfix")))
        ...
        var testInfluencing = [];
        var influencingDecisions = layoutSVG.selectAll("path.clusterLink.target-" + n.id)
                .filter(function(d){ 
                    if("DecRel" == d.type) {
            ...
            var influencingDecisionsUL = $('<ul>').attr("class", "visInfoRelatedNodesUL");     
            
            var show = false;
            influencingDecisions.each(function(d){
            
                influencingDecisionsUL.append($('<li>')
                    .append($('<span>').text(d.source.label).attr("class",function(){
                        return "bordercolorDecision" + d.source.id.toString()[0];
                    })));
                show = true;
            }); 
            
            if(show) {
                var influencingDecisionsInfos = $('<div>').attr("class", "visInfoBOX")
                    .append($('<div>').attr("class", "visInfoHeaderBOX")
                        .append($('<h4>').text("Influencing Decisions"))
                        .append($('<div>').attr("class", "visInfoHeaderToogle")
                            .append($('<a>').attr("href", "#").attr("class", "visInfoHeaderToogleLink").append($('<span>').attr("class", "glyphicon glyphicon-minus"))))
                        .append($('<div>').attr("class", "clearfix")))
    ...
    // Highlight Paths
    var selLinks = layoutSVG.selectAll("path.clusterLink.source-" + n.id + ", path.clusterLink.target-"  + n.id)
            .classed("clusterInfluenceLinkClick", function(d){ return "DecRel" == d.type && layoutSVG.select("#n-"+n.id).classed("noteHighlighted") ? true : false;})
            .classed("clusterAffectationLinkClick", function(d){ return "taskRel" == d.type && layoutSVG.select("#n-"+n.id).classed("noteHighlighted") ? true : false;});
...
function drawForceLayout() {
    ...
    $.ajax({
        ...
        success: function(json) {
           // New Root
           forceNodesTree = {id: -2, type: "root", parent: null, children: []};
           
           json.decisionTree.type = "decRoot";
           forceNodesTree.children = forceNodesTree.children.concat(json.decisionTree.children);
           forceNodesTree.children = forceNodesTree.children.concat(json.taskTree.children);
           var helperC = d3.layout.cluster();
           forceNodesArray = helperC.nodes(forceNodesTree);
           
           forceNodesArray = forceNodesArray.filter(function(d){
    ...
    layoutSVG = d3.select("#"+visContentID)
        .append("svg")
            .attr("width", forceSVGWidth )
            .attr("height", forceSVGHeight );
   
    /* === Init Force === */
    layoutForce = d3.layout.force()
        .size([forceSVGWidth, forceSVGHeight])
        .linkDistance(50)
        .charge(-5000)
        .gravity(1)
        .on("tick", onTick);
    ...
    subDecCount[1] = layoutForce.nodes().filter(function(d){ return "decision" == d.type && d.id.toString()[0] == 1; }).length;
    subDecCount[2] = layoutForce.nodes().filter(function(d){ return "decision" == d.type && d.id.toString()[0] == 2; }).length;
    subDecCount[3] = layoutForce.nodes().filter(function(d){ return "decision" == d.type && d.id.toString()[0] == 3; }).length;
    subDecCount[4] = layoutForce.nodes().filter(function(d){ return "decision" == d.type && d.id.toString()[0] == 4; }).length;
    // TaskCount
    subDecCount[9] = layoutForce.nodes().filter(function(d){ return "task" == d.type; }).length;
    
    /* d3.svg.diagonal.radial Blueprint */
    var diagonal = d3.svg.diagonal.radial().projection(function(d) { return [d.x, d.y]; });

    /* Define Marker */
    layoutSVG.append("defs").selectAll("marker")
          .data(["influenceStart", "influenceEnd", "affectStartDec", "affectEndDec", "affectStartTask", "affectEndTask"]).enter()
                .append("marker")
    ...
    /* === Append Path Links === */
    var pathLinksTaskRels = layoutSVG.append("g").selectAll(".forceLink")
            .data(layoutForce.links().filter(function(d){ return "taskRel" == d.type ;})).enter()
            .append("svg:line")
                .attr("class", function(d) { return "forceLink forceLinkSource-" + d.source.id + " forceLinkTarget-" + d.target.id;})
                .attr("id", function(d) { return "forceLink-" + d.source.id + "-" + d.target.id;})
                .attr("marker-start", function(d) {
                    if("both" == d.dir) {
                        if("decision" == d.source.type) return "url(#affectStartDec)";
                        if("task" == d.source.type) return "url(#affectStartTask)";
                    } else {
                        return null;
                    }
                })
                .attr("marker-end", function(d) {
                    if("decision" == d.target.type) return "url(#affectEndDec)";
                    if("task" == d.target.type) return "url(#affectEndTask)";
                });
    
    var lineLinkHierarchy = layoutSVG.append("g").selectAll(".forceLineLink")
            .data(layoutForce.links().filter(function(d){ return !d.type && d.source.type != "outcome" && d.target.type != "outcome" ;})).enter()
            .append("svg:line")
                .attr("class", "forceLineLink");
             
    /* === Append Nodes === */
    // All nodes
    var forceNodes = layoutSVG.selectAll(".forceNode");
    forceNodes = forceNodes
            .data(layoutForce.nodes()).enter()
            .append("svg:g")
                .attr("id", function(d, i) { return "n-" + d.id; })
          .attr("class", function(d, i) { return "forceNode node " + d.type; })
    .call(layoutForce.drag)
    .on("mouseover", mouseOverForce)
    .on("mouseout", mouseOutForce)
    .on("click", mouseNodeClickForce);    

    
    // Decision Point nodes
    var decisionPoints = forceNodes.filter(function(d){return "decisionPoint" == d.type;});
            decisionPoints.append("svg:rect")
                .attr("id", function(d, i) {return "dp-" + d.id;})
                .attr("class", function(d) {return "forceDecisionPoint decisionPoint"+d.id.toString()[0] ;}) //decisionPoint"+d.id.toString()[0]
                .attr("width", function(d){return (d.id%2)==0 ? dpRectHeight : dpRectWidth;})
                .attr("height", function(d){return (d.id%2)==0 ? dpRectWidth : dpRectHeight;})
                .attr("rx", recRadius)
                .attr("ry", recRadius);
            decisionPoints.append("svg:g")
                .attr("transform", function(d){return (d.id%2)==0 ? "rotate(90)translate(0,-"+dpRectHeight*0.8+")" : "translate(0,"+dpRectHeight*0.2+")";})
                    .append("foreignObject")
                        .attr("width", function(d){return dpRectWidth;})
                        .attr("height", function(d){return dpRectHeight;})
                            .append("xhtml:div")
                                .attr("width", function(d){return dpRectWidth;})
                                .attr("height", function(d){return dpRectHeight;})
                                    .append("xhtml:p")
                                        .attr("class", "forceTextCenter forceTextDP")
                                    .html(function(d){ return d.label;});
                
    
    // Decision nodes            
    var decisions = forceNodes.filter(function(d){return "decision" == d.type;});
            decisions.append("svg:rect")
                .attr("id", function(d, i) {return "d-" + d.id;})
                .attr("class", function(d) {return "forceDecision decision"+d.id.toString()[0] ;}) 
                .attr("width", function(d) {return decRecWidth;})
                .attr("height", function(d) {return decRecHeight;})
                .attr("rx", recRadius)
                .attr("ry", recRadius);
            decisions.append("svg:g")  
                    .append("foreignObject")
                        .attr("width", function(d){return decRecWidth;})
                        .attr("height", function(d){return decRecHeight;})
                            .append("xhtml:div")
                                ...
                                .attr("style", "display: inline-block;")
                                    .append("xhtml:p")
                                        .attr("class", "forceTextCenter forceTextDec")
                                    .html(function(d){ return d.label;});
    
    // Task nodes
    var tasks = forceNodes.filter(function(d){return "task" == d.type;});
           tasks.append("svg:circle")
                .attr("id", function(d, i) {return "t-" + d.id;})
                .attr("class", "forceTask task")
                .attr("r", taskCircleRadius);
           tasks.append("svg:g")
                .attr("transform", "translate(-"+taskCircleRadius+",-"+taskCircleRadius+")")
                    .append("foreignObject")
                        .attr("width", taskCircleRadius*2)
                        .attr("height", taskCircleRadius*2)
                           .append("xhtml:div")
                                .attr("width", "100%")
                                .attr("height", "100%")
                                .attr("style", "display: inline-block;")
                                    .append("xhtml:p")
                                        .attr("class", "forceTextCenter forceTextTask")
                                    .html(function(d){ return d.label;});
    ...
    function onTick() {
        ...
        pathLinksTaskRels
            .attr("x1", function(d) { return ("decision" == d.source.type) ? (d.source.x + decRecWidth/2) : d.source.x; })
            .attr("y1", function(d) { return ("decision" == d.source.type) ? (d.source.y + decRecHeight/2) : d.source.y; })
            .attr("x2", function(d) { return ("decision" == d.target.type) ? (d.target.x + decRecWidth/2) : d.target.x; })
            .attr("y2", function(d) { return ("decision" == d.target.type) ? (d.target.y + decRecHeight/2) : d.target.y; });
    ...
    function transformNodes(d) {
        ...
        if("decisionPoint" == d.type) {
            if((d.id%2)==0) {
                ...
                if(d.id == 2) d.x = forceSVGWidth - dpPadding - dpRectHeight;
                if(d.id == 4) d.x = dpPadding;
            } else {
        ...
        if("decision" == d.type) {
            ...
            if((d.parent.id%2) != 0) {
                ...
                if(d.parent.id == 1) d.y = decisionPaddingVertical;
                if(d.parent.id == 3) d.y = forceSVGHeight - decisionPaddingVertical ;

                // Fix Positioning
                if(d.parent.id == 1) {
                    var partitions1 = totalRange / subDecCount[1];
                    if(d.id == 101) d.x = minRange + partitions1/2;
                    if(d.id == 102) d.x = minRange + partitions1 + partitions1/2;
                    if(d.id == 103) d.x = minRange + partitions1*2 + partitions1/2;
                    if(d.id == 104) d.x = minRange + partitions1*3 + partitions1/2;   
                }
                
                if(d.parent.id == 3) { 
                    var partitions3 = totalRange / subDecCount[3];
                    if(d.id == 301) d.x = minRange + partitions3/2;
                    if(d.id == 302) d.x = minRange + partitions3 + partitions3/2;
                }
                ...
                var totalRange = maxRange - minRange;
                if(d.parent.id == 2) d.x = forceSVGWidth - decisionPaddingHorizontal; 
                if(d.parent.id == 4) d.x = decisionPaddingHorizontal; 
                
                // Fix Positioning
                if(d.parent.id == 2) { 
                    var partitions2 = totalRange / subDecCount[2];
                    if(d.id == 201) d.y = minRange + partitions2/2;
                    if(d.id == 202) d.y = minRange + partitions2 + partitions2/2;
                    if(d.id == 203) d.y = minRange + partitions2*2 + partitions2/2;
                    if(d.id == 204) d.y = minRange + partitions2*3 + partitions2/2;  
                }  
                
                if(d.parent.id == 4) { 
                    var partitions4 = totalRange / subDecCount[4];
                    if(d.id == 401) d.y = minRange + partitions4/2;
                    if(d.id == 402) d.y = minRange + partitions4 + partitions4/2;
                    if(d.id == 403) d.y = minRange + partitions4*2 + partitions4/2;
                    if(d.id == 404) d.y = minRange + partitions4*3 + partitions4/2;  
                    if(d.id == 405) d.y = minRange + partitions4*4 + partitions4/2;
                    if(d.id == 406) d.y = minRange + partitions4*5 + partitions4/2;
                    if(d.id == 407) d.y = minRange + partitions4*6 + partitions4/2;
                }
        ...
        if("task" == d.type) {
            ...
            function calcCircleX(taskD, circlePartition) {
                return taskCircleCenterX + taskCircleRadius*Math.cos(circlePartition);
            }
            
            function calcCircleY(taskD, circlePartition) {
                return taskCircleCenterY + taskCircleRadius*Math.sin(circlePartition);
            }
            
            // Fix Positioning
            d.x = calcCircleX(d, taskCirclePartion*parseInt(d.id.toString().substr(1,2))); 
            d.y = calcCircleY(d, taskCirclePartion*parseInt(d.id.toString().substr(1,2)));
    ...
    function setForceLegend() {
        ...
        legendItems.append($('<span>').attr("class", "legendItem")
            .append($('<img>').attr("src", "./img/legends/force-dpDecLink.png"))
            .append($('<span>').text("Decision Point - Decision Link"))
            );
        legendItems.append($('<span>').attr("class", "legendItem")
            .append($('<img>').attr("src", "./img/legends/force-affectationLink.png"))
...
function setForceDefaultInformation() {
    ...
    var defaultInformation = $('<div>').attr("class", "visInfoBOX")
            .append($('<p>').attr("class", "defaultInformation")
            .text("Hover over an element to highlight their relationships and related elements. Click on one or more elements to either select or deselect it and its relationships."))
...
function setForceSettings() {
    ...
    var displayedNodes = $('<div>').attr("class", "visInfoBOX")
        .append($('<div>').attr("class", "visInfoHeaderBOX")
        ...
        .append($('<div>').attr("class", "visInfoContentBOX")
            ...
            .append($('<div>').attr("class", "input-group input-group-sm visInputBox")
                .append($('<span>').attr("class", "input-group-addon")
                    .append($('<input>').attr("type", "checkbox").attr("name", "showDecisions").attr("id", "showDecisions").attr("checked", function(){return layoutForceDefaultSettings["showDecisions"];})))
                .append($('<span>').attr("class", "form-control").text("Decisions"))
            )
            .append($('<div>').attr("class", "input-group input-group-sm visInputBox")
                .append($('<span>').attr("class", "input-group-addon")
                    .append($('<input>').attr("type", "checkbox").attr("name", "showTasks").attr("id", "showTasks").attr("checked", function(){return layoutForceDefaultSettings["showTasks"];})))
                .append($('<span>').attr("class", "form-control").text("Tasks"))
            )
        
        );

    var displayedLinks = $('<div>').attr("class", "visInfoBOX")
        .append($('<div>').attr("class", "visInfoHeaderBOX")
        ...
        .append($('<div>').attr("class", "visInfoContentBOX")
            ...
            .append($('<div>').attr("class", "input-group input-group-sm visInputBox")
                .append($('<span>').attr("class", "input-group-addon")
                    .append($('<input>').attr("type", "checkbox").attr("name", "showInfluenceLinks").attr("id", "showInfluenceLinks").attr("checked", function(){return layoutForceDefaultSettings["showInfluenceLinks"];})))
                .append($('<span>').attr("class", "form-control").text("Influences"))
            )          
            .append($('<div>').attr("class", "input-group input-group-sm visInputBox")
                .append($('<span>').attr("class", "input-group-addon")
                    .append($('<input>').attr("type", "checkbox").attr("name", "showAffectLinks").attr("id", "showAffectLinks").attr("checked", function(){return layoutForceDefaultSettings["showAffectLinks"];})))
                .append($('<span>').attr("class", "form-control").text("Affectation"))
            )
            
            .append($('<div>').attr("class", "btn-group btn-group-sm btn-group-justified")
                .append($('<div>').attr("class", "btn-group btn-group-sm")
                    .append($('<button>').attr("type", "button").attr("class", "btn btn-default").attr("name", "btn").attr("id", "btn").text("Affectation")))
                .append($('<div>').attr("class", "btn-group btn-group-sm")
                    .append($('<button>').attr("type", "button").attr("class", "btn btn-default active").attr("name", "btn").attr("id", "btn").text("Influences")))
            )
    ...
    var clearHighlight = $('<div>').attr("class", "visInfoBOX")
        ...
        .append($('<div>').attr("class", "visInfoContentBOX")
            ...
            .append($('<div>').attr("class", "visInputBox")
                .append($('<button>').attr("type", "button").attr("class", "btn btn-default btn-sm btn-block").attr("id", "clearForceHighlights").text("Clear Highlights"))   
            )
        );
    ...
    var information = $('<div>').attr("class", "visInfoBOX")
        ...
        .append($('<div>').attr("class", "visInfoContentBOX")
            ...
            .append($('<div>').attr("class", "input-group input-group-sm visInputBox")
                .append($('<span>').attr("class", "input-group-addon")
                    .append($('<input>').attr("type", "checkbox").attr("name", "showLabels").attr("id", "showLabels").attr("checked", function(){return layoutForceDefaultSettings["showLabels"];})))
                .append($('<span>').attr("class", "form-control").text("Show Labels"))
            )
        );
    ...
    $('#showLabels').change(function(){
        if($(this).prop("checked")){
            $('.forceTextDP, .forceTextDec, .forceTextTask').css("display", "block");
        } else {
...
function mouseNodeClickForce(n) {
    ...
    // Set Related Tasks
    var relTasks = layoutForce.links().filter(function(d){return ("task" == d.source.type || "task" == d.target.type) && (d.source.id == n.id || d.target.id == n.id);});
    if (relTasks.length > 0) {
        var relatedTasksLIs = $('<ul>').attr("class", "visInfoRelatedNodesUL");
        
        $.each(relTasks, function(i,v){
        
             relatedTasksLIs.append($('<li>')
                .append($('<span>')
                .text(function(){
                    return v.source.id != n.id ? v.source.label : v.target.label;
                })
                .attr("class", function(){
                    if(v.source.id != n.id) {
                        if("task" == v.source.type) return "bordercolorTask";
                        if("decision" == v.source.type) return "bordercolorDecision" + v.source.id.toString()[0];
                    } else {
                        if("task" == v.target.type) return "bordercolorTask";
                        if("decision" == v.target.type) return "bordercolorDecision" + v.target.id.toString()[0];
                    } 
                })
                    .append($('<span>').attr("class", "affectDirection").text(function(){
                        if("auto" == v.dir) {
        ...
        var relatedNodesInfos = $('<div>').attr("class", "visInfoBOX")
            .append($('<div>').attr("class", "visInfoHeaderBOX")
                .append($('<h4>').text(relatedLabel))
                .append($('<div>').attr("class", "visInfoHeaderToogle")
                            .append($('<a>').attr("href", "#").attr("class", "visInfoHeaderToogleLink").append($('<span>').attr("class", "glyphicon glyphicon-minus"))))
    ...
    var selLinks = layoutSVG.selectAll(".forceLinkSource-" + n.id + ", .forceLinkTarget-" + n.id);
        ...
        selLinks.each(function(d){
            ...
            if("taskRel" == d.type) {
                if(layoutSVG.select("#n-"+n.id).classed("noteHighlighted")) {
                    if("decision" == d.source.type) layoutSVG.select("#d-"+d.source.id).classed("forceHighlightNode", true);
                    if("decision" == d.target.type) layoutSVG.select("#d-"+d.target.id).classed("forceHighlightNode", true);
                    if("task" == d.source.type) layoutSVG.select("#t-"+d.source.id).classed("forceHighlightNode", true);
                    if("task" == d.target.type) layoutSVG.select("#t-"+d.target.id).classed("forceHighlightNode", true);
                } else {
                    if("decision" == d.source.type) layoutSVG.select("#d-"+d.source.id).classed("forceHighlightNode", false);
                    if("decision" == d.target.type) layoutSVG.select("#d-"+d.target.id).classed("forceHighlightNode", false);
                    if("task" == d.source.type) layoutSVG.select("#t-"+d.source.id).classed("forceHighlightNode", false);
                    if("task" == d.target.type) layoutSVG.select("#t-"+d.target.id).classed("forceHighlightNode", false);
                }
            }
        });
...
function clearForceHighligths() {
    // Clear all highlights in force layout
    layoutSVG.selectAll('.forceHighlightNode, .noteHighlighted, .forceAffectationLinkHighlightClick, .forceAffectationLinkHighlightHover')
        .classed("forceHighlightNode", false)
        .classed("noteHighlighted", false)
        .classed("forceAffectationLinkHighlightClick", false)
...
function mouseOverForce(d) {
    layoutSVG.selectAll(".forceLinkSource-" + d.id + ", .forceLinkTarget-" + d.id)
        .classed("forceAffectationLinkHighlightHover", function(d){return "taskRel" == d.type ? true : null;});
        //.classed("forceAffectationLinkHighlight", function(d){return "taskRel" == d.type ? true : null;})
}

function mouseOutForce(d) {
    layoutSVG.selectAll(".forceLinkSource-" + d.id + ", .forceLinkTarget-" + d.id)
        .classed("forceAffectationLinkHighlightHover", function(d){return "taskRel" == d.type ? false : null;});
}
...
var svg = d3.select("#"+visContentID).append("svg")
    .attr("class", "overflowHidden")
...
d3.json("./js/json/elaboratedDSF.json", function(root) {
  ...
  function display(d) {
    ...
    g.append("text")
        .attr("dy", ".75em")
        .attr("class", function(d){ return "decisionPoint" == d.type ? "treemapDPNodeText" : "treemapNodeText"; })
        .text(function(d) { return d.label; })
        .call(text);
...
function setTreemapSettings() {
    ...
    var information = $('<div>').attr("class", "visInfoBOX")
        ...
        .append($('<div>').attr("class", "visInfoContentBOX")
            ...
            .append($('<div>').attr("class", "input-group input-group-sm visInputBox")
                .append($('<span>').attr("class", "input-group-addon")
                    .append($('<input>').attr("type", "checkbox").attr("name", "showLabels").attr("id", "showLabels").attr("checked", function(){return layoutTreemapDefaultSettings["showLabels"];})))
                .append($('<span>').attr("class", "form-control").text("Show Labels"))
            )
    );
    $('#visPanelSettingsContent').prepend(information);
